...
echo "========================================"
echo ""
echo "WARNING: This will wipe all existing VM data!"
echo "  - VM overlay disks will be recreated"
echo "  - Config disks will be regenerated"
echo "  - TAP devices will be recreated"
echo "  - All logs and snapshots will be deleted"
echo ""
...
echo "[INFO] Creating fresh hosts file..."
...
for i in $(seq 1 $NUM_VMS); do
  ...
  # Wipe existing VM directory if it exists
  if [ -d "$VM_DIR" ]; then
    echo "       Wiping existing VM data..."
    rm -rf "$VM_DIR"
  fi
  
  # Create fresh VM directory structure
  mkdir -p "$VM_DIR/logs"
  ...
  DOCKER_METADATA="$BASE_DIR/images/chromium-headful/v1/metadata.json"
  DOCKER_WORKDIR=$(jq -r '.[].Config.WorkingDir // "/"' "$DOCKER_METADATA")
  DOCKER_ENTRYPOINT=$(jq -r '.[].Config.Entrypoint | if . == null then "" else join(" ") end' "$DOCKER_METADATA")
  DOCKER_CMD=$(jq -r '.[].Config.Cmd | if . == null then "" else join(" ") end' "$DOCKER_METADATA")
  
  # Start building config.sh
...
# Docker image environment variables (from metadata)
EOF
  
  # Extract and add environment variables from Docker metadata
  jq -r '.[].Config.Env[]' "$DOCKER_METADATA" 2>/dev/null | while read -r env_line; do
    echo "export $env_line" >> "$CONFIG_DIR/config.sh"
  done
  
  # Add runtime-specific overrides and additional envs
  cat >> "$CONFIG_DIR/config.sh" <<EOF
...
# Entrypoint configuration (from Docker metadata)
...
EOF
  ...
  if ip link show "$TAP" &>/dev/null; then
    echo "       Removing existing TAP device..."
...
echo ""
echo "All VMs have fresh overlay disks and configs from Docker metadata."
echo ""
